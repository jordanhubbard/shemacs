#!/usr/bin/env zsh
# em.scm.zsh - Launcher for the Scheme-powered shemacs editor (zsh native)
#
# Source this file in your .zshrc:  source /path/to/em.scm.zsh
# Then run:  em-scm [filename]
# Or run standalone:  zsh em.scm.zsh [filename]
#
...
if [[ -n "${_SHEMACS_SCM_ZSH_LOADED:-}" && "${(%):-%N}" != "$0" ]]; then
    return 0 2>/dev/null
fi
_SHEMACS_SCM_ZSH_LOADED=1
...
em-scm() {
    ...
    if (( ${#_em_bs_candidates[@]} == 0 )); then
        print "em-scm: cannot find bs.sh — install sheme: https://github.com/jordanhubbard/sheme" >&2
        return 1
    fi
    ...
    if [[ -f "$HOME/.em.scm" ]]; then
        ...
        print "em-scm: cannot find em.scm" >&2
        return 1
    fi
    ...
    if [[ -f "$_em_cache_file" \
       ...
       && source "$_em_cache_file" && (( ${+functions[em_main]} )); then
        ...
        if [[ ! -f "$_em_cache_file" ]]; then
            printf "em-scm: First run — compiling Scheme to zsh (this may take a moment).\n" >&2
            printf "em-scm: Subsequent runs use the cache and start instantly.\n" >&2
        else
            printf "em-scm: Cache is stale; recompiling.\n" >&2
        fi
        ...
        if [[ -z "$_em_compiled" ]]; then
            print "em-scm: bs.sh found but bs-compile-zsh missing — update sheme." >&2
            return 1
        fi
...
# Standalone execution: zsh em.scm.zsh [filename]
if [[ "${(%):-%N}" == "$0" ]]; then
    em-scm "$@"
fi
